...
library(dplyr)
# Read data
data <- read.csv("reallifedata.csv", header = TRUE)
names(data)
# Functions needed
...
for (model_used in model_list) {
  ...
  test_connection()
  data <- read.csv("location.csv", header = TRUE) #change dataset if necessary
...
#################     Extracting Names      #########################
  descriptions <- as.list(data$Description[grepl("Yes", data$explicit.people.indicator.chat)])
  for(i in seq_along(descriptions2)) {
    description_text <- descriptions2[[i]]
    description_name <- names(descriptions2)[i]
    setTimeLimit(elapsed = 180, transient = TRUE)
...
#################     Extracting Relationship label      #########################
  descriptions <- as.list(data$Description[grepl("[A-Za-z]", data$explicit.names.chat)])
  for(i in seq_along(descriptions2)) {
    description_text <- descriptions2[[i]]
    description_name <- names(descriptions2)[i]
    setTimeLimit(elapsed = 180, transient = TRUE)
  ...
  write.csv(data, paste0("reallife_results_", model_file_tag, ".csv"), row.names = FALSE)
